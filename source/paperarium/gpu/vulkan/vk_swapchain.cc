...
#endif
{
  ...
  VK_CHECK_RESULT(fpGetPhysicalDeviceSurfaceFormatsKHR(physicalDevice, surface,
                                                       &formatCount, NULL));
 ...
 *
 */
void VKSwapchain::connect(VkInstance instance, VkPhysicalDevice physicalDevice,
                          VkDevice device) {
 ...
 */
VkResult VKSwapchain::acquireNextImage(VkSemaphore presentCompleteSemaphore,
                                       uint32_t* imageIndex) {
 ...
 */
VkResult VKSwapchain::queuePresent(VkQueue queue, uint32_t imageIndex,
 ...
 */
void VKSwapchain::createDirect2DisplaySurface(uint32_t width, uint32_t height) {
  ...
  for (uint32_t i = 0; i < displayPropertyCount; ++i) {
    ...
    for (uint32_t j = 0; j < modeCount; ++j) {
      VkDisplayModePropertiesKHR const* mode = &pModeProperties[j];
