...
VkShaderModule loadShader(AAssetManager* assetManager, char const* fileName,
                          VkDevice device) {
...
#else
VkShaderModule loadShader(char const* resourcePath, VkDevice device) {
  QByteArray shaderData = VulkanEngine::QtTools::readResource(resourcePath);
  assert(shaderData.size() > 0);
  VkShaderModule shaderModule;
  VkShaderModuleCreateInfo moduleCreateInfo{};
  moduleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
  moduleCreateInfo.codeSize = shaderData.size();
  moduleCreateInfo.pCode = (uint32_t*)shaderData.data();
  VK_CHECK_RESULT(
      vkCreateShaderModule(device, &moduleCreateInfo, NULL, &shaderModule));
  return shaderModule;
}
