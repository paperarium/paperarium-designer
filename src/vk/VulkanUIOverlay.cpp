...
#if defined(__ANDROID__)
  float scale =
      (float)vks::android::screenDensity / (float)ACONFIGURATION_DENSITY_MEDIUM;
  AAsset* asset =
...
#else
  QByteArray fontFileBuf =
      VulkanEngine::QtTools::readResource(":/fonts/Roboto-Medium.ttf");
...
bool UIOverlay::update() {
  ...
  for (int n = 0; n < imDrawData->CmdListsCount; n++) {
    ImDrawList const* cmd_list = imDrawData->CmdLists[n];
...
void UIOverlay::draw(const VkCommandBuffer commandBuffer) {
  ...
  for (int32_t i = 0; i < imDrawData->CmdListsCount; i++) {
    ImDrawList const* cmd_list = imDrawData->CmdLists[i];
    for (int32_t j = 0; j < cmd_list->CmdBuffer.Size; j++) {
      ImDrawCmd const* pcmd = &cmd_list->CmdBuffer[j];
...
bool UIOverlay::header(char const* caption) {
  return ImGui::CollapsingHeader(caption, ImGuiTreeNodeFlags_DefaultOpen);
}

bool UIOverlay::checkBox(char const* caption, bool* value) {
...
bool UIOverlay::checkBox(char const* caption, int32_t* value) {
  bool val = (*value == 1);
...
bool UIOverlay::inputFloat(char const* caption, float* value, float step,
                           char const* format) {
...
bool UIOverlay::sliderFloat(char const* caption, float* value, float min,
                            float max) {
...
bool UIOverlay::sliderInt(char const* caption, int32_t* value, int32_t min,
                          int32_t max) {
...
bool UIOverlay::comboBox(char const* caption, int32_t* itemindex,
...
bool UIOverlay::button(char const* caption) {
...
void UIOverlay::text(char const* formatstr, ...) {
  va_list args;
