...
/**
 * @brief Global vulkan instance for handling windows procedures
 */
VulkanBase* g_vulkanInstance;
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARM wParam, LPARAM lParam) { \
  if (g_vulkanInstance != NULL) {                                             \
    g_vulkanInstance->handleMessages(hWnd, uMsg, wParam, lParam);             \
  }                                                                           \
  return (DefWindowProc(hWnd, uMsg, wParam, lParam));                         \
}

void VulkanBase::setWindow(uint32_t winId) {
	m_window = static_cast<HWND>(winId);
}
