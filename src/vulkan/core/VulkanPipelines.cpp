 ...
 * @param renderPass 
 */
void VulkanPipelines::createBasePipelineInfo(const VkPipelineLayout &pipelineLayout, const VkRenderPass &renderPass) {
  m_inputAssemblyState = vks::initializers::pipelineInputAssemblyStateCreateInfo(
...
void VulkanPipelines::createPipeline(VulkanShader *shader, VkPolygonMode mode) {
  if (shader) {
    ...
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(m_device, m_pipelineCache, 1,
      &m_pipelineCreateInfo, nullptr, &(shader->getPipeline())));
  }
}
